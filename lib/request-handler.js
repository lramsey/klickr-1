...
exports.handleGetKeystrokes = function (req, res) {
  // need to query all data in db and send it back to client
  Keystroke.findById(req.params.id, function (err, keystroke) {
    if (!!err) {
      console.log("Error in Getkeystrokes");
      throw new Error('error', err);
    }
    console.log(keystroke);
    res.send(200, keystroke);
  });
}

exports.handlePostKeystrokes = function (req, res) {
  Keystroke.create(req.body.keystroke, function(err, obj){
    if(!!err){
      console.log("Error in Postkeystrokes");
      throw new Error('error', err);
    }
    console.log(obj);
    res.send(200, obj);
